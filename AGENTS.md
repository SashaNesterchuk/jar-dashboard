...
Never commit secrets. This app depends on `AUTH_SECRET`, `OPENAI_API_KEY`, `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `POSTHOG_HOST`, `POSTHOG_PROJECT_ID`, and `POSTHOG_API_KEY`. The collaborative doc editor uses a separate Supabase project (`NEXT_PUBLIC_DOCS_SUPABASE_URL`, `NEXT_PUBLIC_DOCS_SUPABASE_ANON_KEY`, server-only `DOCS_SUPABASE_SERVICE_ROLE_KEY`) and optional `ANTHROPIC_API_KEY` for Claude models in `/api/collab-docs/chat`. Store them in local environment files and document any new variables in the PR.
