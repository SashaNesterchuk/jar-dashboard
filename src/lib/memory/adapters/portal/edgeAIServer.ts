...
async function callServerEdge<T>(
  operation: Op,
  ...
  if (!supabaseUrl || !serviceKey) {
    throw new Error("[memory.ai.edge.server] missing SUPABASE env vars");
  }

  const response = await fetch(`${supabaseUrl}/functions/v1/generate_ai_memory`, {
    method: "POST",
    headers: {
  ...
  if (!response.ok) {
    throw new Error(
      `[memory.ai.edge.server] ${operation} failed (${response.status}): ${JSON.stringify(parsed)}`,
    );
  }
  return (parsed ?? {}) as T;
}
...
export const edgeServerAIAdapter: AIAdapter = {
  async generateSmartSummary(input: SmartSummaryInput): Promise<SmartSummaryOutput> {
    const data = await callServerEdge<Record<string, unknown>>(
      "smart_summary",
